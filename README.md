...
Đề bài nói password nằm trong file `data.txt` , là kí tự đọc được và đứng đằng sau vài dấu `=`. Đầu tiền , `ls` ra thì có duy nhất 1 file `data.txt` . Dùng lệnh `file` để xem dạng file của file này thì thấy là 1 file data(không thể đọc bằng `cat`). Ta dùng `strings` để đọc những file data như này và kết hợp với `grep` để tìm ra những dấu `=`. Lọc ra được vài kết quả thì
